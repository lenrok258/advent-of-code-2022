...
enc_file = [int(n) * dec_key for n in lines]

def print_ll(ll_first_node):
    l = []
    node = ll_first_node
    for _ in enc_file:
        l.append(f"{node.val :>13}")
        node = node.nxt
    print(" ".join(l))
...
# Mixing
mixing_offset = len(enc_file) - 1

for i_mix in range(10):
    for e_i, e_val in enumerate(enc_file):
        ...
        ne_node = e_node
        if e_val > 0:
            for i in range(abs(e_val) % mixing_offset):
                ne_node = ne_node.nxt
